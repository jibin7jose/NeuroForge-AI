...
function getAiEngineUrl(): string {
    const configured = vscode.workspace.getConfiguration('neuroforge').get<string>('aiEngineUrl');
...
class NeuralSidebarProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'neuroforge.sidebar';
    ...
    private _getHtmlForWebview(data?: any) {
        ...
        const issuesHtml = weaknesses.slice(0, 4).map((w: any) => `
...
class NeuralChatProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'neuroforge.chat';
    private _view?: vscode.WebviewView;
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    const applyFixDisposable = vscode.commands.registerCommand('neuroforge.applyFix', async (diagnostic: vscode.Diagnostic) => {
        ...
        if (diagnostic.message.includes('boundary') || diagnostic.message.includes('security')) strategyId = 'refactor-security-rigidity';
        if (diagnostic.message.includes('loop') || diagnostic.message.includes('nest')) strategyId = 'refactor-entropy-reduction';
        ...
        vscode.window.withProgress({
            ...
            try {
                ...
                const confirm = await vscode.window.showInformationMessage(
                    `Neural Patch Preview: ${linesChanged} line${linesChanged !== 1 ? 's' : ''} will change. Apply?`,
                    { modal: false },
                    'Apply Patch',
                    'Cancel'
                );
                        ...
                        <div class="container">
                            ...
                            <div style="margin-top: 20px;">
                                ...
                                <div style="margin-top: 10px;">
                                    ${traits.map((t: string) => `<span class="trait-tag">${t}</span>`).join('')}
    ...
    const interviewDisposable = vscode.commands.registerCommand('neuroforge.interviewMode', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) { vscode.window.showWarningMessage('Open a file to start Interview Mode.'); return; }
        ...
        const panel = vscode.window.createWebviewPanel(
            'neuroforgeInterview',
            'NeuroForge: Elite Interview Mode',
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                try {
                    ...
                    if (weaknesses.length > 0) {
                        const diags: vscode.Diagnostic[] = weaknesses.map((w: any) => {
                            const r = new vscode.Range(0, 0, 0, doc.lineAt(0).range.end.character);
                            const d = new vscode.Diagnostic(r,
                                `[NeuroForge] ${w.area}: ${w.message}`,
                                w.risk === 'High' ? vscode.DiagnosticSeverity.Error : vscode.DiagnosticSeverity.Warning
                            );
                            d.source = 'NeuroForge';
    ...
    const heatmapDisposable = vscode.commands.registerCommand('neuroforge.showHeatmap', async () => {
        ...
        const panel = vscode.window.createWebviewPanel(
            'neuroforgeHeatmap',
            'NeuroForge: Neural Complexity Heatmap',
            vscode.ViewColumn.Two,
            { enableScripts: true }
        );
    ...
    const generateTestsDisposable = vscode.commands.registerCommand('neuroforge.generateTests', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor || !editor.document.getText().trim()) return;
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('neuroforge.healthCheck', checkHealth)
    );
}
